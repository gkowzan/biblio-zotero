...
;; URL: https://github.com/gkowzan/biblio-zotero
...
;; `biblio-zotero-insert-bibtex'. The command will insert a formatted BibTeX
;; entry in the current buffer. Information is retrieved from an URL by asking a
;; Zotero translation server to extract bibliographic data from the location.
;; `biblio-zotero-translation-server' has to contain the address of a running
;; server instance (see [zotero-server] for details). The default value points
;; to localhost. The easiest way to run your local instance is to set up
;; [docker] and run the server with:
;;
;; ```sh
...
;;
;; [zotero-server]: https://github.com/zotero/translation-server
